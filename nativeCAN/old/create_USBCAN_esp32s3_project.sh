...
PROJECT_NAME="esp32_s3_can_v30_bidirectional"

if [ -d "$PROJECT_NAME" ]; then rm -rf $PROJECT_NAME; fi
mkdir -p $PROJECT_NAME/main

echo "Creating v30 (Full Bi-Directional Support)..."
...
// 2. DATA OUT (Host -> Device -> CAN) -- NEW IN V30
void tud_vendor_rx_cb(uint8_t itf) {
    if (!is_can_started) { tud_vendor_read_flush(); return; }
    
    struct gs_host_frame frame;
    
    // Read all available frames from USB buffer
    while (tud_vendor_available() >= sizeof(frame)) {
        if (tud_vendor_read(&frame, sizeof(frame)) == sizeof(frame)) {
            ...
            // Put onto Physical Bus
            // We use DontBlock because this runs in USB Task context. 
            // If CAN bus is full, we drop packet rather than freezing USB.
            twai_transmit(&msg, 0); 
        }
    }
}
...
// 3. CAN RX Task (Bus -> Queue)
void can_rx_task(void *arg) {
    twai_message_t msg; 
    struct gs_host_frame frame;
    ESP_LOGI(TAG, "CAN RX Active");
...
void app_main(void) {
    ESP_LOGI(TAG, "=== v30 BI-DIRECTIONAL FINAL ===");
    pending_mode.flags = MAGIC_FLAG;
    can_to_usb_queue = xQueueCreate(64, sizeof(struct gs_host_frame));
